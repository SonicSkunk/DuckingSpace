...
#include "src/VerbHall.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct VerbHallMetaData {
		SN_NODE_ID("VerbHall");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using VerbHall = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _VerbHall, VerbHallMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros
