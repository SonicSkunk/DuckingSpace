...
#include "src/VerbScript.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct VerbScriptMetaData {
		SN_NODE_ID("VerbScript");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using VerbScript = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _VerbScript, VerbScriptMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros
